...
export function useWallet(): WalletState {
  ...
  const connect = useCallback(async () => {
    const eth = getEth();
    if (!eth) {
      setError('No wallet detected. Install a GenLayer compatible wallet to enter the arena.');
      return;
    }
    setConnecting(true);
    setError(null);
    try {
      ...
      if (/reject|denied|4001/i.test(msg)) setError('You waved off the connection');
      else setError('Could not reach the wallet');
    } finally {
      setConnecting(false);
    }
  }, [refreshChain]);
